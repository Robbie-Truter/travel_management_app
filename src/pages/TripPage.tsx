...
import { AccommodationCard, AccommodationForm, AccommodationComparison } from '@/components/accommodations/AccommodationComponents'
...
export function TripPage() {
  ...
  // Accommodations
  const { accommodations, addAccommodation, updateAccommodation, deleteAccommodation, confirmAccommodation } = useAccommodations(id)
  ...
  // Activities
  const { activities, addActivity, updateActivity, deleteActivity, reorderActivities } = useActivities(id)
          ...
          >
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    {flights.length >= 2 && (
                      <Button variant="secondary" size="sm" onClick={() => setFlightCompareOpen(true)}>
                        <BarChart2 size={14} />Compare
                      </Button>
                    )}
                    <Button variant="primary" size="sm" onClick={() => { setEditingFlight(undefined); setFlightFormOpen(true) }}>
                      <Plus size={14} />Add Flight
                    </Button>
                  </div>
                </div>
                {flights.length === 0 ? (
                  <EmptyState icon={Plane} label="No flights yet" action={() => setFlightFormOpen(true)} actionLabel="Add Flight" />
                ) : (
                    ...
                    <AnimatePresence>
                      {flights.map(f => (
                        <FlightCard
                          key={f.id}
                          flight={f}
                          onEdit={(fl: Flight) => { setEditingFlight(fl); setFlightFormOpen(true) }}
                ...
                <FlightForm
                  ...
                  onSave={editingFlight?.id
                    ? async (data) => { await updateFlight(editingFlight.id!, data) }
                    : addFlight
                  }
                  initial={editingFlight}
                  tripId={id}
                />
                <FlightComparison open={flightCompareOpen} onClose={() => setFlightCompareOpen(false)} flights={flights} />
              </div>
            )}
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="primary" size="sm" onClick={() => { setEditingAcc(undefined); setAccFormOpen(true) }}>
                      <Plus size={14} />Add Stay
                    </Button>
                  </div>
                </div>
                {accommodations.length === 0 ? (
                  <EmptyState icon={Hotel} label="No accommodations yet" action={() => setAccFormOpen(true)} actionLabel="Add Stay" />
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    <AnimatePresence>
                      {accommodations.map(a => (
                        <AccommodationCard
                          key={a.id}
                          acc={a}
                          onEdit={(ac: Accommodation) => { setEditingAcc(ac); setAccFormOpen(true) }}
                ...
                <AccommodationForm
                  ...
                  onSave={editingAcc?.id
                    ? async (data) => { await updateAccommodation(editingAcc.id!, data) }
                    : addAccommodation
                  }
                  initial={editingAcc}
                  tripId={id}
                />
                <AccommodationComparison open={accCompareOpen} onClose={() => setAccCompareOpen(false)} accommodations={accommodations} />
              </div>
            )}
            ...
            {activeTab === 'activities' && (
              <div>
                ...
                {activities.length === 0 ? (
                  <EmptyState icon={Compass} label="No activities yet" action={() => setActFormOpen(true)} actionLabel="Add Activity" />
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    <AnimatePresence>
                      {activities.map(a => (
                        <ActivityCard
                          key={a.id}
                          activity={a}
                          onEdit={(act: Activity) => { setEditingAct(act); setActFormOpen(true) }}
                ...
                <ActivityForm
                  ...
                  onSave={editingAct?.id
                    ? async (data) => { await updateActivity(editingAct.id!, data) }
                    : addActivity
                  }
                  initial={editingAct}
                  tripId={id}
