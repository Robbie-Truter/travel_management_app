...
export async function importTripFromJSON(file: File): Promise<number> {
  ...
  await Promise.all([
    ...flights.map(({ id: _id, ...f }) =>
      db.flights.add({ ...f, tripId: idMap[f.tripId] ?? newTripId }),
    ),
    ...accommodations.map(({ id: _id, ...a }) =>
      db.accommodations.add({ ...a, tripId: idMap[a.tripId] ?? newTripId }),
    ),
    ...activities.map(({ id: _id, ...a }) =>
      db.activities.add({ ...a, tripId: idMap[a.tripId] ?? newTripId }),
    ),
    ...notes.map(({ id: _id, ...n }) =>
      db.notes.add({ ...n, tripId: idMap[n.tripId] ?? newTripId }),
    ),
  ]);
