...
export function PlannerTimeline({
  ...
  if (JSON.stringify(activities.map(a => a.id)) !== JSON.stringify(localActivities.map(a => a.id))) {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    onReorderActivities(updated.filter(a => a.id !== undefined).map(a => ({ id: a.id!, order: a.order })))
  }
                  ...
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={cn(
                        ...
                        snapshot.isDraggingOver ? 'bg-sage-50 dark:bg-sage-900/20 border-2 border-dashed border-sage-300' : 'border-2 border-dashed border-transparent'
                      )}
                    >
...
function StaticPlannerCard({ item }: { item: PlannerItem }) {
  const icons = {
    flight: { icon: Plane, color: 'text-sky-pastel-500', bg: 'bg-sky-pastel-100 dark:bg-sky-pastel-900/30' },
    accommodation: { icon: Hotel, color: 'text-peach-500', bg: 'bg-peach-100 dark:bg-peach-900/30' },
    activity: { icon: Compass, color: 'text-lavender-500', bg: 'bg-lavender-100 dark:bg-lavender-900/30' },
  }
