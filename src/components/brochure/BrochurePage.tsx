    ...
    async function loadData() {
      if (selectedTripId) {
        ...
        if (trip) {
          ...
          setTripData({ trip, flights, accommodations, activities });
        }
        setLoadingPdf(false);
      } else {
        setTripData(null);
  ...
  return (
    <div className="flex-1 overflow-auto bg-surface-2 p-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Create Brochure</h1>
          <p className="text-text-secondary">Generate a PDF travel brochure from your trips</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-4">
          {!selectedTripId ? (
            ...
            <div className="bg-surface rounded-xl border border-border overflow-hidden flex flex-col h-[700px]">
              <div className="p-4 border-b border-border flex justify-between items-center bg-surface-2">
                <h3 className="font-semibold text-text-primary">PDF Preview</h3>
                <PDFDownloadLink
