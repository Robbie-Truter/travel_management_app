...
import { Plane, DollarSign, ExternalLink, Edit, Trash2, CheckCircle, ArrowRight } from 'lucide-react'
  ...
  return (
    <>
      <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
        <Card className={flight.isConfirmed ? 'border-sage-300 dark:border-sage-700' : ''}>
          ...
          <CardFooter className="justify-end">
            {!flight.isConfirmed && (
              <Button variant="ghost" size="sm" onClick={() => onConfirm(flight.id!)} className="text-sage-600">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { onDelete(flight.id!); setDeleteOpen(false) }}
        title="Delete Flight"
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <Input id="fl-airline" label="Airline" placeholder="e.g. Emirates" value={form.airline} onChange={e => set('airline', e.target.value)} error={errors.airline} />
          <Input id="fl-num" label="Flight Number" placeholder="e.g. EK201" value={form.flightNumber} onChange={e => set('flightNumber', e.target.value)} error={errors.flightNumber} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Input id="fl-dep-ap" label="Departure Airport" placeholder="e.g. JFK" value={form.departureAirport} onChange={e => set('departureAirport', e.target.value)} error={errors.departureAirport} />
          <Input id="fl-arr-ap" label="Arrival Airport" placeholder="e.g. DXB" value={form.arrivalAirport} onChange={e => set('arrivalAirport', e.target.value)} error={errors.arrivalAirport} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Input id="fl-dep-t" label="Departure Time" type="datetime-local" value={form.departureTime} onChange={e => set('departureTime', e.target.value)} error={errors.departureTime} />
          <Input id="fl-arr-t" label="Arrival Time" type="datetime-local" value={form.arrivalTime} onChange={e => set('arrivalTime', e.target.value)} error={errors.arrivalTime} />
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {[
              ...
              { label: 'Status', render: (f: Flight) => f.isConfirmed ? '✅ Confirmed' : 'Option' },
